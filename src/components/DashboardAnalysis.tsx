...
export default function DashboardAnalysis() {
  ...
  const runAnalysis = async (toolId) => {
    ...
    if (toolId === "hotspot") {
      let pts = []; 
      ["2024","2023","2022","2021"].forEach(y => { if(data[`bencana-banjarnegara-${y}.geojson`]) pts.push(...data[`bencana-banjarnegara-${y}.geojson`].features); });
      const coll = turf.featureCollection(pts);
      let res = [];
      for(let i=0; i<vGeo.features.length; i++) {
        if(stopRef.current) break;
        res.push({ id: vGeo.features[i].properties.OBJECTID, val: turf.pointsWithinPolygon(coll, vGeo.features[i]).features.length });
      ...
      for(let i=0; i<vGeo.features.length; i++) {
        if(stopRef.current) break;
        let s = 0; try { if(turf.booleanIntersects(vGeo.features[i], buff)) s += 50; } catch(e){}
      ...
      for(let i=0; i<roads.features.length; i++) {
        if(stopRef.current) break;
        let risk = 0; try { if(turf.booleanIntersects(roads.features[i], buff)) risk = 1; } catch(e){}
  ...
  const handleVillageClick = (f) => {
    let ls = 0;
    ["2024","2023","2022","2021"].forEach(y => {
      const d = data[`bencana-banjarnegara-${y}.geojson`];
      if(d) ls += d.features.filter(p => turf.booleanPointInPolygon(p, f)).length;
    });
    const soil = data["tanah-jateng.geojson"]?.features?.find(s => turf.booleanPointInPolygon(turf.centroid(f), s));
      ...
      <aside className="w-80 bg-white border-r p-5 shadow-xl z-20 flex flex-col gap-4 overflow-y-auto">
        ...
        {!activeTool ? (
          ...
          <div className="space-y-4 animate-in slide-in-from-right-4">
            <div className="flex justify-between border-b pb-2"><h3 className="font-black text-red-900 uppercase text-xs">{TOOLS.find(t=>t.id===activeTool).title}</h3><button onClick={()=>{setActiveTool(null); setResults(null); setRoadRisk(null); setVillageStats(null);}} className="text-[9px] font-bold text-slate-400 uppercase">Tutup</button></div>
            
            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200"><h5 className="text-[9px] font-black text-slate-500 uppercase mb-1 underline">Keterangan</h5><p className="text-[10px] text-slate-600 italic">{TOOLS.find(t=>t.id===activeTool).desc}</p></div>
        ...
        <MapContainer center={[-7.36, 109.68]} zoom={11} minZoom={9} className="h-full w-full" zoomControl={false} zoomSnap={0.25} zoomDelta={0.25}>
          ...
          {data["peta_desa.geojson"] && (
            <GeoJSON key={"desa-"+activeTool+(results?"-an":"")} data={data["peta_desa.geojson"]} 
              style={(f) => {
                if (results) {
                  const r = results.find(x => x.id === f.properties.OBJECTID);
          ...
          {roadRisk && data["jalan.geojson"] && <GeoJSON key="an-infra" data={data["jalan.geojson"]} style={(f,i) => ({ color: roadRisk[i]?.atRisk ? "#dc2626" : "#f59e0b", weight: roadRisk[i]?.atRisk?4:1, opacity: roadRisk[i]?.atRisk?1:0.2 })} />}
        </MapContainer>
      </main>
    </div>
  );
